...
|---|---|---|---|---|---|
| `c2st/acc` ↓ | 0.998 | 0.998 | 0.579 | 0.594 | →0.5 = indistinguishable from real |
| `c2st/auc` ↓ | 1.000 | 1.000 | 0.615 | 0.619 | →0.5 |
| `c2st/graph_acc` ↓ | 1.000 ⚠ | 0.999 | 0.610 | 0.609 | →0.5 (spatial-graph view) |
| `c2st/graph_auc` ↓ | 1.000 ⚠ | 1.000 | 0.664 | 0.652 | →0.5 |
| `c2st/pos_acc` ↓ | 0.634 ⚠ | 0.498 | 0.593 | 0.581 | →0.5 (position-only) |
| `mmd2/x` ↓ ⚠ | 0.369 | 0.339 | 0.002 | 0.003 | →0 = matched expression marginals |
| `mmd2/pos` ↓ | 0.073 ⚠ | −0.000 | 0.012 | 0.028 | →0 |
| `ot_w1/x` ↓ ⚠ | 29.07 | 28.80 | 5.53 | 5.21 | →0 (PCA-scale dependent) |
| `ot_w2/x` ↓ ⚠ | 35.94 | 35.57 | 5.65 | 5.38 | →0 |
| `ot_w1/pos` ↓ | 0.281 ⚠ | 0.049 | 0.226 | 0.214 | →0 |
| `ot_w2/pos` ↓ | 0.337 ⚠ | 0.058 | 0.282 | 0.277 | →0 |
...
> cannot see. For **NicheFlow CFM/VFM** the graph C2ST stays near chance (≈0.61–0.66 AUC), only
...
| `spd/mean` ↓ | 0.013 ⚠ | 0.012 | 0.020 | 0.014 | real cloud is covered |
| `spd/max` ↓ | 0.044 ⚠ | 0.101 | 0.374 | 0.225 | low worst-case gap |
...
| `moran/gen_mean` → real_mean | −0.000 | 0.249 | 0.104 | 0.175 | match `real_mean` |
| `moran/corr` ↑ | 0.096  | 0.735 | 0.925 | 0.981 | →1 (per-gene match) |
| `moran/mae` ↓ | 0.257 | 0.078 | 0.085 | 0.023 | →0 |

## Cell-type classifier `ct/*` (classifier trained on slide 1.026)

| Metric | OT-CFM gene-only | OT-CFM spatial | NicheFlow CFM (unaligned) | NicheFlow VFM | Expected |
|---|---|---|---|---|---|
| `ct/acc_real` ⚠ | 0.524 | 0.625 | 0.281 | 0.292 | classifier sanity on real niches |
| `ct/acc_gen` ↑ | 0.340 | 0.594 | 0.318 | 0.308 | high = generated classifiable |
| `ct/acc_gap` ↓ | 0.184 | 0.031 | 0.037 | 0.016 | →0 = gen as classifiable as real |
| `ct/acc` ↑ | 0.531 | 0.777 | 0.630 | 0.606 | →1 = labels agree with paired-real |
| `ct/f1` ↑ | 0.368 | 0.733 | 0.591 | 0.571 | →1 |
| `ct/prop_kl` ↓ | 7.31 | 0.368 | 0.304 | 0.158 | →0 = composition matches |
| `ct/prop_tv` ↓ | 0.469 | 0.151 | 0.170 | 0.140 | →0 |
| `ct/prop_jsd` ↓ | 0.199 | 0.030 | 0.024 | 0.016 | →0 |

## Regression — matched ground truth (niche-shaped models only)

| Metric | OT-CFM gene-only | OT-CFM spatial | NicheFlow CFM (unaligned) | NicheFlow VFM | Expected |
|---|---|---|---|---|---|
| `x/mae` ↓ | — (skipped) | — (skipped) | 1.059 | 1.062 | →0 vs matched real niche |
| `x/mse` ↓ | — (skipped) | — (skipped) | 1.919 | 1.959 | →0 |
| `pos/mae` ↓ | — (skipped) | — (skipped) | 0.535 | 0.532 | →0 |
| `pos/mse` ↓ | — (skipped) | — (skipped) | 0.467 | 0.451 | →0 |
...
- **NicheFlow (CFM & VFM) dominate the distribution match.** Both are near-inseparable from real on
  every C2ST view (`c2st/auc` ≈ 0.61–0.62, `c2st/graph_auc` ≈ 0.65–0.66), with tiny `mmd2/x`
  (≈0.002–0.003), while both OT-CFM variants sit at ~1.0 separability and ~100× larger `mmd2/x`.
