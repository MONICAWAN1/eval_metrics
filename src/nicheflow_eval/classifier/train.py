...
from nicheflow_eval.utils.log import RankedLogger, log_hyperparameters, print_config
from nicheflow_eval.utils.seed import set_seed

_logger = RankedLogger(__name__, rank_zero_only=True)


# Usage: python -m nicheflow_eval.classifier.train
@hydra.main(config_path="../../../configs", config_name="train", version_base=None)
@print_exceptions
def main(config: DictConfig) -> float | None:
    # Use TF32 tensor cores for fp32 matmuls (Ampere+ GPUs, e.g. A100). Harmless
    # on CPU/older GPUs and complements bf16-mixed for any non-autocast matmuls.
