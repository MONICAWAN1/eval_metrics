...
— not the target's own annotations. See ``nicheflow_eval.classifier`` for training one.
"""
...
from nicheflow_eval.metrics._common import (
...
def _resolve_n_neighbors(n_neighbors: int | None, classifier) -> int:
    ...
    :func:`~nicheflow_eval.metrics._common.load_spatial_classifier`); otherwise fall back to
