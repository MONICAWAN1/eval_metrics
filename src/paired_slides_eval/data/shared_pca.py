...
@dataclass
class SharedGenePCA:
    """The frozen gene -> whitened-X_pca transform, re-applicable to new gene-
    space cells.
    ...
    def components(self) -> np.ndarray:
        """``(n_pcs, n_genes)`` — the ``_PCA``-style components used for space
        auto-detection."""
        return np.asarray(self.pcs, dtype=np.float64).T

    def align_genes(self, genes: np.ndarray, input_var_names) -> np.ndarray:
        """Reorder ``genes`` columns to the fit ``var_names`` (no-op when
        already aligned / unknown)."""
    ...
    def transform(self, genes: np.ndarray) -> np.ndarray:
        """Project ``genes`` ``(cells, n_genes)`` (in fit ``var_names`` order)
        into whitened X_pca."""
    ...
    def transform_lognorm(self, genes: np.ndarray) -> np.ndarray:
        """Project **already log-normalised** ``genes`` ``(cells, n_genes)``
        into whitened X_pca.
...
def shared_pca_from_dataclass(ds, *, apply_lognorm: bool = True) -> SharedGenePCA:
    """Build a :class:`SharedGenePCA` from a preprocessed-slide dataclass (or
    raise if it can't).
...
def coord_standardizer_from_dataclass(ds, timepoint: str | None = None) -> CoordStandardizer:
    """Build a :class:`CoordStandardizer` from the dataclass's
    ``stats['coords'][timepoint]``.
...
class Basis:
    ...
    def from_dataclass(
        ...
        *,
        ...
        """Reconstruct the basis from a preprocessed pair pickle (the shared PCA
        + coord stats it stores)."""
        return cls(
    ...
    def apply(self, genes=None, coords=None):
        """Put a slide into the shared space. Returns ``(X_pca, coords_std)``;
    ...
    def to_fm_npz(self, out_npz: str) -> dict:
        """Write the basis as an ``fm_mnist`` ``load_spatial_pca`` stats
        ``.npz`` (to train a model in it).
