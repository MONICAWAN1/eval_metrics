...
def _pca_aware_transform(x2d: np.ndarray, pca) -> np.ndarray:
    """Project ``x2d`` (rows = cells) into the target ``pca`` basis, auto-
    detecting its space.
...
class TargetSlide:
    ...
    def moran_grid(self) -> tuple[np.ndarray, np.ndarray]:
        """The (x, pos) used as the Moran's I real reference — the full target
        cloud.

        Moran's I now scores **all** generated cells against the full real
        slide, so there is no density-matched grid subsample any more; this
        returns the whole cloud.
    ...
    def from_anndata(
        ...
        *,
        ...
        """Build a ``TargetSlide`` from an original AnnData slide (raw genes +
        coordinates).
    ...
    def from_dataclass(
        ...
        *,
        ...
        """Build a ``TargetSlide`` from a **preprocessed-slide pickle** (an
        ...
        if shared_pca:  # True or "auto"
            try:
                ...
                pca = coord_transform = (
                    None  # "auto" on a pre-recipe pickle -> plain reduced target
                )
...
class GeneratedNiches:
    ...
    def to_slide(self) -> GeneratedSlide:
        """Flatten the niches into a :class:`GeneratedSlide` — all niche points
        pooled.
    ...
    def flat_x(self) -> np.ndarray:
        """All generated cells as one ``(B*N, n_features)`` cloud (see
        :meth:`to_slide`)."""
        return self.to_slide().x

    @property
    def flat_pos(self) -> np.ndarray:
        """All generated cells pooled into one ``(B*N, coord)`` cloud (via
        :meth:`to_slide`)."""
    ...
    def project(self, pca) -> GeneratedNiches:
        """Project expression (and ``gt_x``) through a target ``pca`` into the
        shared basis.
    ...
    def from_anndata(
        ...
        *,
        ...
        """Build ``(B, N, D)`` niches from a flat generated AnnData (the
        pipeline's output).
...
class GeneratedSlide:
    ...
    @property
    def flat_x(self) -> np.ndarray:
        """All generated cells as an ``(N, n_features)`` cloud (already
        flat)."""
        return self.x
    ...
    def to_slide(self) -> GeneratedSlide:
        """Return ``self`` — a flat slide is already flat (symmetry with
        ``GeneratedNiches``)."""
        return self

    def project(self, pca) -> GeneratedSlide:
        """Project expression into the target's basis, or pass through if
        already PCA-reduced.
    ...
    def from_anndata(
        ...
        *,
        ...
        """Build a flat ``GeneratedSlide`` from a generated AnnData (one row per
        cell).
