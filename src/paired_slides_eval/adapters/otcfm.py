...
The OT-CFM is **unconditional and expression-only** (it generates whitened-PCA expression, no spatial
coordinates). So ``source`` is ignored, and the generated cells are placed at **placeholder**
coordinates drawn from the reference (target) slide. The expression metrics (``distribution``,
``c2st``) are the meaningful ones; the spatial metrics (``moran``/``psd``/``spd``) reflect only that
the baseline has no spatial structure — score with ``--groups distribution c2st`` for the honest read.
...
class OTCFMGenerator(BaseGenerator):
    ...
    Construction parameters (from ``configs/generator/otcfm.yaml``):
        ...
        coord_mode: placeholder coordinates — ``"reference"`` (resample the reference positions) or
            ``"random"`` (uniform over the reference bounding box). The OT-CFM models no geometry.
    ...
    def __call__(self, *, source, target, checkpoint, **_) -> GenerationOutput:
        ...
        try:
            from fm.data import invert_pca_expression, sample_prior
        ...
        # Sample: prior -> velocity-field ODE -> whitened PCA -> invert to gene counts.
        ...
        gen_counts = invert_pca_expression(gen.cpu().numpy(), stats)  # (n_gen, n_model_genes)
