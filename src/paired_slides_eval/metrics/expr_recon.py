"""Gene-expression reconstruction gap (``recon/mse_real``, ``recon/mse_gen``,
``recon/mse_gap``).
...
def _predict_center(regressor, feats: np.ndarray) -> np.ndarray:
    """Run the frozen regressor over masked-centroid neighbour sets -> predicted
    centre expression."""
    import torch
    ...
    with torch.no_grad():
        return regressor(torch.as_tensor(feats, dtype=torch.float32, device=device)).cpu().numpy()
...
def expr_recon_gap(
    ...
    *,
    ...
    """Reconstruction MSE of ``regressor`` on real vs. generated niches, and the
    gap.
...
def fixed_reference_mse(
    ...
    *,
    ...
    """Reconstruction MSE on a **model-independent** seeded sample of real
    target niches.
